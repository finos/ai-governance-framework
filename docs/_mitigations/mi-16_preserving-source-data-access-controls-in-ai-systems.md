...
* **Source-Side Reconciliation (Detecting Bypass of the Enforcement Path):** Configuration audits, access log monitoring, entitlement reviews and lineage tracking all read records the AI system itself produces; penetration testing probes for weaknesses at a point in time. Neither surfaces an access that already occurred without passing the enforcement point: a component connecting to the source with its own credential, a scheduled job outside the governed path, or a logging sink that silently stopped writing. A hash-chained or signed log detects tampering with the records it holds; it is silent about records that were never created. Detecting this class of failure requires a second account of activity that the AI system does not produce. Where a source maintains its own activity accounting (for example PostgreSQL `pg_stat_statements`, database audit logs, or object-store access logs), snapshot it at the boundaries of a review window and compare what the source recorded against what the AI system recorded for the same window. Activity the source accounted for that no AI-side record explains is the detective signal this control asks for.
    * **Report absence as absence.** A missing record is evidence that evidence is absent, not evidence of intent — bypass, sink failure, and accounting-scope mismatch produce the same observation. Attributing cause is investigation, not reconciliation.
    * **Do not compare counts one to one.** One client-level request may legitimately produce several source-level statements (connection poolers, ORMs, query rewriting), so counts must not be equated; match on the objects touched. A window whose counters moved backwards indicates the source's accounting was reset or otherwise altered and should be reported as unreliable in its entirety rather than reconciled in part.
    * **State the scope.** This method reaches only sources that expose usable accounting, and it is not a defense against an operator who can suppress that accounting — it raises the cost and makes resets visible. Where the AI system and the source are operated by the same party, that limit belongs in the control's assessment rather than being assumed away.
    * **Two clocks, one window.** The source's accounting and the AI system's records are timestamped by different clocks. An AI-side record that genuinely covers an access can fall outside the review window purely because the two clocks disagree, manufacturing an accusation where no gap exists. Report the distance from the boundary rather than judging it, and treat an item that is unexplained only because of the window edge as indeterminate rather than as absent evidence. Where a tolerance is declared, state its source and its bound; an item outside the declared tolerance is a finding, not a clock artefact. A discrepancy larger than the review window itself cannot be a boundary effect at all, whatever tolerance is declared.
